...
import (
	...
	"github.com/haitao-sun03/donation/backend-end/user/common"
	...
	"github.com/haitao-sun03/donation/backend-end/user/service"
	"github.com/haitao-sun03/donation/backend-end/user/utils"
	"github.com/haitao-sun03/donation/backend-end/user/vo"
)
...
func (au AuthController) GetUserRole(ctx *gin.Context) {
	...
	authService := service.NewAuthService()
	// 查询针对某address是否拥有某campaignId的某权限
	exists := authService.GetUserRole(userRoleVO.Address, userRoleVO.Role, userRoleVO.CampaignId)
...
func (au AuthController) AddUserRole(ctx *gin.Context) {
	...
	authService := service.NewAuthService()
	// 查询某用户目前所有角色
	err := authService.AddUserRole(userRoleVO.Address, userRoleVO.Role, userRoleVO.CampaignId)
	if err != nil {
		common.Fail(ctx, http.StatusInternalServerError, err)
		return
	}

	common.Success(ctx, http.StatusOK, common.MessageSuccess, nil, -1)
}
